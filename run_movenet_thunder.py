...
def load_img(path_to_img):
    ...
    img = tf.image.resize(img, [256, 256])  # Resize dimensions if needed for Thunder
...
def run_pose_estimation_video(frame):
    ...
    start_time = time.time()
    frame_resized = cv2.resize(frame, (128, 128))  # Downscale frame for faster processing
    frame_rgb = cv2.cvtColor(frame_resized, cv2.COLOR_BGR2RGB)
    img_data = tf.image.resize(frame_rgb, [256, 256])
...
def process_video(video_path):
    ...
    while cap.isOpened():
        ...
        if not ret:
            ...
            break
        if frame_count % skip_frames == 0:  # Skip frames to improve processing time
            keypoints, elapsed_time = run_pose_estimation_video(frame)
            frame = draw_skeleton_on_frame(frame, keypoints, POSE_PAIRS)
...
def main():
    image_path = 'C:/Users/shrut/OneDrive/Desktop/Final Project - CV/img.jpg'
